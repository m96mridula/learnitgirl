...
class tweetProcess:
	def __init__(self):
		self.root = r"F:\project\traitsPredictor-master"
		self.process = r"F:\project\traitsPredictor-master\process"
	...
	def readFiles(self, filename, values):
		with open(self.root+"\\"+filename, "r") as f:
			...
			for row in f:
				...
				for word in row.translate(str.maketrans('','', string.punctuation)).split():
